...
const bgPresets = [
    ...
    { id: 'grassland', label: '大草原', css: 'linear-gradient(180deg, #87CEEB 0%, #87CEEB 45%, #90B77D 45%, #6B8E23 100%)' }
];
...
function setupBookmarkEvents() {
    ...
    content.addEventListener('click', function(e) {
        ...
        if (!target) {
            ...
            if (card && !card.classList.contains('add-card') && card.dataset.id) {
                var bm = bookmarks.find(function(b) { return b.id === parseInt(card.dataset.id); });
                if (bm) openBookmark(bm.url);
            }
            return;
        }
    ...
    content.addEventListener('mousemove', function(e) {
        if (!dragStartPos || !dragStartCard) return;
        var dx = Math.abs(e.clientX - dragStartPos.x);
        var dy = Math.abs(e.clientY - dragStartPos.y);
        if (dx > 5 || dy > 5) {
            isDragging = true;
            draggedItem = dragStartCard;
            dragStartCard.classList.add('dragging');
        }
    });
...
function loadBackgroundImage() {
    ...
    if (currentBgStyle === 'bg.JPG' || !currentBgStyle) {
        var img = new Image();
        img.onload = function() {
